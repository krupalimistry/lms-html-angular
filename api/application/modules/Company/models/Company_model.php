	...
	public function get_companydata($company_id=Null)
	{
	  if($company_id)
	  {
		...
		// $result = $this->db->get('tblmstcompetencyarea area');
		 $this->db->select('cp.CompanyId,cp.Name,cp.EmailAddress,cp.Address,cp.IndustryId,cp.Website,cp.PhoneNo,cp.IsActive,(SELECT COUNT(u.UserId) FROM tbluser as u WHERE u.CompanyId=cp.CompanyId) as isdisabled');
